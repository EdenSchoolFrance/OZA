...
@section('content')
    <div class="content users">
        <div class="card card--add_work_unit">
            <form class="card-body" action="{{ route('admin.help.danger.update', [$danger->id]) }}" method="POST">
                @csrf
                <div class="row">
                    <div class="line">
                        <div class="left">
                        ...
                        <div class="right">
                            <input type="text" name="name" id="name" class="form-control @error('name') invalid @enderror" placeholder="Nom du danger" value="{{ old('name') ? : $danger->name }}" >
                            @error('name')
                    ...
                    <div class="line">
                        ...
                        <div class="right">
                            <textarea name="info_danger" id="info_danger" class="form-control @error('info_danger') invalid @enderror" placeholder="Marie">{{ old('info_danger') ? : $danger->info }}</textarea>
                            @error('info_danger')
                                <p class="message-error">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div class="line line--reflection">
                        ...
                        <div class="right">
                            <ul class="ul-textarea">
                                @if(old('reflection'))
                                    @foreach(old('reflection') as $reflection)
                                        <li>
                                ...
                                @else
                                    @foreach($danger->reflections as $reflection)
                                        <li>
                                            <button type="button" class="btn btn-text btn-small btn-delete"><i class="far fa-times-circle"></i></button>
                                            <textarea class="form-control auto-resize" placeholder="" name="reflection[]">{{ $reflection->name }}</textarea>
                                        </li>
                                    @endforeach
                                @endif
                                <li>
